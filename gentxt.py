...
class Chatterer:
    ...
    def prepare(self):
        self.totals = {}
        self.stats_for_generate = {}
        for sequence, char_stat in self.stat.items():
            self.totals[sequence] = 0
            self.stats_for_generate[sequence] = []
            for char, count in char_stat.items():
                self.totals[sequence] += count
                self.stats_for_generate[sequence].append([count, char])
            self.stats_for_generate[sequence].sort(reverse=True)
    ...
    def chat(self, N, out_file_name=None):
        ...
        spaces_printed = 0
        while printed < N:
            char = self._get_char(char_stat=self.stats_for_generate[sequence], total=self.totals[sequence])
            if file:
                file.write(char)
            else:
                print(char, end='')
